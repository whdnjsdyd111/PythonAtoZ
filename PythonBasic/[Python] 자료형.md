...
불(bool) 은 참과 거짓을 나타내는 자료형이다.
파이썬에선 `True` 와 `False` 와 같이 첫 문자를 항상 대문자로 사용한다.

```python
>>> a = True
>>> b = False

>>> 1 == 1 # 이와 같은 조건문도 불을 반환
True
```

파이썬에선 다음과 같이 참과 거짓을 구분하는 기준이 있다.
...
`bool` 내장 함수를 통해 자료형의 참과 거짓을 판별할 수도 있다.
...
변수는 저장할 값이나 객체를 가리키는 것이다.
특히 객체는 어떠한 속성값이나 행동을 가지고 있는 모든 데이터(숫자, 문자, 함수)를 뜻한다.
지금까지 배운 것으로 리스트, 튜플, 딕셔너리, 집합 등이 되겠다.

각 객체는 자동으로 메모리에 생성되어 각각의 주소를 갖는다.
`id` 내장 함수를 이용하면 주소를 확인할 수 있다.
...
b 변수에 a 변수를 단순히 대입하니 같은 객체를 참조하고 있다는 것을 알 수 있다.
`is` 명령어를 실행하여도 동일한 객체인지 확인할 수 있다.
...
a 리스트의 변화가 b 에도 동일하게 일어났는데, 이는 동일한 리스트를 바라보고 있기 때문이다.
원본은 그대로 두면서 서로 다르게 주소를 가리키도록 해보자.
...
`copy` 모듈을 이용한 방법으로, 모듈을 불러와 사용하는 방법이다.
...
위 방법은 `b = a.copy()` 와 같은 리스트 자체 함수를 사용해도 copy 모듈과 동일하다.
...
위와 같이 튜플 형태로도 a,b 에 값을 대입할 수 있다.

```python
>>> a = b = 'python'
```

또한, 위와 같이 같은 값을 여러 개 변수에 대입할 수도 있다.
위와 같은 방법들로 두 변수의 값을 서로 쉽게 바꿀 수 있다.
